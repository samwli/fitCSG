...
def sdf_ellipsoid(params, color, points, all_params):
    center, sizes, _ = params['center'], params['sizes'], params['axis']
    
    if not all_params:
        sizes = sizes.detach()
    sizes = sizes.clamp(min=0.01)  # Ensure sizes are non-zero
...
def sdf_prism(params, color, points, all_params):
...
def sdf_sphere(params, color, points, all_params):
...
def sdf_cylinder(params, color, points, all_params):
...
def sdf_cone(params, color, points, all_params):
    # Cylinder parameters: [center_x, center_y, center_z, radius, height, axis_x, axis_y, axis_z]
    center, radius, height, axis = params['center'], params['radius'], params['height'], params['axis']
    axis = axis / torch.linalg.norm(axis)  # Normalize the axis vector
    axis = torch.tensor([0, 0, 1]).to(points.device)
