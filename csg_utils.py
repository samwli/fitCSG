...
def construct_sdf(tree, leaf_params, points, all_params, get_colors=False):
    if isinstance(tree, str):  # Leaf node case
        if get_colors:
            color = list(filtered_colors.values())[int(tree[-1])]
        else:
            color = None
        return sdf_dict[tree[:-1]](leaf_params[tree], color, points, all_params)
    else:  # Recursive case for operations
        left_sdf, left_color = construct_sdf(tree['left'], leaf_params, points, all_params, get_colors)
        right_sdf, right_color = construct_sdf(tree['right'], leaf_params, points, all_params, get_colors)
        ...
        if operation == 'union':
            ...
            final_sdf = torch.maximum(left_sdf, -right_sdf)
            if get_colors:
                color_mask = torch.ones_like(left_sdf, dtype=torch.bool)
